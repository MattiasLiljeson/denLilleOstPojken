...
Avatar* GOFactory::CreateAvatar(Tilemap* p_map, Tile* p_startTile, GameStats* p_stats)
{
	...
	return new Avatar(spriteInfo, p_map, p_startTile, p_stats,
		...
		CreateSoundInfo("../Sounds/jump.wav",100) );
}
...
Pill* GOFactory::CreatePill(Tile* p_tile, GameStats* p_gameStats)
{
	...
	return new Pill(spriteInfo,
		CreateSoundInfo("../Sounds/new_eat_sound_2012-10-16.wav", 100),
		p_tile, p_gameStats);
}
BombPill* GOFactory::CreateBombPill(Tile* p_tile, GameStats* p_gameStats)
{
	...
	SpriteInfo* spriteInfo = CreateSpriteInfo("../Textures/Item_Dynamite.png",
		pos, size, NULL);
	return new BombPill(spriteInfo, p_tile, p_gameStats,
		CreateSoundInfo("../Sounds/use_power-up.wav", 100) );
}
Bomb* GOFactory::CreateBomb(Tile* p_tile, Tilemap* p_map)
{
	...
	TilePosition dir[] =
	{
		...
		TilePosition(0, 1), TilePosition(0, -1)
	};
...
Tilemap* GOFactory::CreateTileMap(int p_width, int p_height, bool* p_initData)
{
	...
	for (int row = 0; row < p_height; row++)
	{
		...
		{
			tiles[row * p_width + col] = CreateTile(
				p_initData[row * p_width + col],
...
GUI* GOFactory::CreateGUI(GameStats* p_gameStats)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		fVector3 pos = fVector3(1.1f * size.x * i + size.x*0.5f,
			height - 0.08f * height*0.5f, 0.9f); 
	...
	fVector2 size = fVector2(50*widthFraction, 50*heightFraction);
	fVector3 pos = fVector3(widthFraction*1920*0.5f,
		height - 0.08f * height*0.5f, 0.9f); 
	...
	pos = fVector3(widthFraction*1920- widthFraction*350,
		height - 0.08f * height*0.5f, 0.9f); 
	string xtext = "X";
	...
	size = fVector2(50*widthFraction, 50*heightFraction);
	pos = fVector3(widthFraction*1920- widthFraction*300,
		height - 0.08f * height*0.5f, 0.9f); 
	...
	size = fVector2(50*widthFraction, 50*heightFraction);
	pos = fVector3(widthFraction*1920- widthFraction*150,
		height - 0.08f * height*0.5f, 0.9f); 
	string ytext = "Z";
	...
	size = fVector2(50*widthFraction, 50*heightFraction);
	pos = fVector3(widthFraction*1920- widthFraction*100,
		height - 0.08f * height*0.5f, 0.9f); 
